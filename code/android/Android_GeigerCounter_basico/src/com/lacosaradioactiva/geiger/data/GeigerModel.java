...
public class GeigerModel extends Observable {
	// TODO: improve precision of calculations? (cpm should be calculated then
	// in float, not int)
	...
	class Q extends LinkedList<Integer> {
		...
		public boolean add(Integer i) {
			...
			if (size < maxSize) {
				...
				size++;
				average = (i + average * (size - 1)) / size; // Update average
																// value
	...
	static final float CONVERSION_FACTOR = (float) 0.00812037037037; // Geiger
																		// Tube
																		// Conversion
																		// Factor.
																		// Hardcoded:
																		...
																		// make
																		// configurable?
	// Conversion factor from manufacturer
	...
	static final float SAMPLE_INTERVAL = 0.500f; // (secs) Hardcoded. TODO: make
													// configurable?
	private static final int INTEGRATION_INTERVAL = 60; // In seconds
	// private static final int MAX_QUEUE_SIZE = (int) (60 /
	// MainActivity.SAMPLE_INTERVAL); // Correspond to 1 min
	private static final int MAX_QUEUE_SIZE = (int) (INTEGRATION_INTERVAL / SAMPLE_INTERVAL); // Primary
																								// queue
	private static final int MAX_QUEUE_SIZE_2 = 10; // Num of intervals
													// averaged, for secondary
													// queue

	private final Q q_long_term = new Q(60);
	private LinkedList<Integer> q1; // FIFO to hold cpm measures got from
									// Arduino
	private final Q q = new Q(MAX_QUEUE_SIZE); // Main queue for holding primary
												// measured raw values
	private final Q q2 = new Q(MAX_QUEUE_SIZE_2); // Secondary queue for
	...
	private void calculateModel() {
		...
		if (seqNum % (60 / SAMPLE_INTERVAL) == 0) { // long term value each
													// minute
